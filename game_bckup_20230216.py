    ...
    def _robot_player_move(self): 
        ...
        if n == 1: # first move
            ...
            for i in range(n):
                ...
                # diagonal 1 defence
                elif HUMAN.move[i][0] == HUMAN.move[i][1] and \
                        HUMAN.move[i - 1][0] == HUMAN.move[i - 1][1]:
                    for z in range(BOARD_SIZE):
                        if self.board[z][z] == "-": # checking for a empty spot
                            self._update_move(ROBOT, z, z)
                
                # diagonal 2 defence
                elif HUMAN.move[i][0] == (BOARD_SIZE - 1 - HUMAN.move[i][1]) and \
                        HUMAN.move[i - 1][0] == (BOARD_SIZE - 1 - HUMAN.move[i - 1][1]):
                    for z in range(BOARD_SIZE):
